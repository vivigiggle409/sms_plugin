...
include APP_PATH.'plugin/alimarket_mobile/model/sms.func.php';
...
// 发送验证码
	...
	if($action2 == 'user_create') {
		...
		define('APP_PATH','../../../');
		//判断验证码插件是否开启
		$conffile = APP_PATH."plugin/xn_vcode/conf.json";
		$exists = is_file($conffile);
		if ($exists) $plugin_vcode = xn_json_decode(file_get_contents($conffile));
		//获取短信先判断图形验证码
		$kv_vcode = kv_get('vcode');
		if($exists && $plugin_vcode['installed'] == 1 && $plugin_vcode['enable'] == 1 && $kv_vcode['vcode_user_create_on'] == 1) {
		...
		//判断验证码插件是否开启
		$conffile = APP_PATH."plugin/GG_vcode/conf.json";
		$exists = is_file($conffile);
		if ($exists) $plugin_vcode = xn_json_decode(file_get_contents($conffile));
		//获取短信先判断图形验证码
		$GG_vcode_setting = kv_get('GG_vcode');
		if($exists && $plugin_vcode['installed'] == 1 && $plugin_vcode['enable'] == 1 && $GG_vcode_setting['register']==1){
