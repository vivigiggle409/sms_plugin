...
function send_sms($mobile, $message, $word = 0, $time = 0) {
	...
	if(strlen(DT_CLOUD_UID) >= 8 && strlen(DT_CLOUD_KEY) == 32){
		...
		$request->setQuery("content", $sms_message);
		...
		if($response->getSuccess()){
			...
			$code = $response->getHttpStatusCode() . ' '. ($response->getErrorMessage()==""?json_decode($response->getBody())->errmsg:$response->getErrorMessage());
		}
	}else{
		$sendurl = "http://api.chanyoo.cn/utf8/interface/send_sms.aspx?username=".DT_CLOUD_UID."&password=".DT_CLOUD_KEY."&receiver=".$mobile."&content=".$sms_message."";
